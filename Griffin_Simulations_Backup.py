...
def fetch_stock_data(ticker):
    ...
    # Calculate daily log returns: Close - Open
...
# Monte Carlo simulation function for stock price
def monte_carlo_simulation(S0, mu, sigma, T, N, n_simulations):
    dt = T / N
    ...
    for sim in range(n_simulations):
        for t in range(1, N):
            ...
            price_paths[t, sim] = price_paths[t-1, sim] * np.exp((mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * z)
...
st.write("""
    None of the information or recomendations you receive from this website should be listened to, 
...
# Heston model Monte Carlo simulation for stock prices
def heston_model_simulation(S0, mu, sigma, T, N, kappa, theta, xi, rho, n_simulations):
    dt = T / N
    price_paths = np.zeros((N, n_simulations))
    vol_paths = np.zeros((N, n_simulations))  # Volatility paths
    price_paths[0] = S0
    vol_paths[0] = sigma  # Start with current volatility
    ...
    for sim in range(n_simulations):
        for t in range(1, N):
            ...
            z2 = rho * z1 + np.sqrt(1 - rho**2) * z2  # Correlating the second random variable
            
            # Stochastic volatility update (variance process)
            vol_paths[t, sim] = max(vol_paths[t-1, sim] + kappa * (theta - vol_paths[t-1, sim]) * dt 
                                    + xi * np.sqrt(vol_paths[t-1, sim]) * np.sqrt(dt) * z2, 0)
            
            # Stock price update using the Heston model
            price_paths[t, sim] = price_paths[t-1, sim] * np.exp((mu - 0.5 * vol_paths[t, sim]) * dt 
                                                                + np.sqrt(vol_paths[t, sim]) * np.sqrt(dt) * z1)
